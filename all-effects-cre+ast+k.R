...
variables <- c('OUT', 'AGE', 'SCNS', 'BLDING', 'AST', 'CRE', 'K')
yvar <- 'OUT'
model_string <- 'OUT~AGE+SCNS+BLDING+AST+CRE+K'
model_name <- 'all-effects-cre+ast+k' 
...
imp_models <- with(imp_data, glm(family="binomial", 
                                 formula=OUT~AGE+SCNS+BLDING+AST+CRE+K))
